...
class SoftmaxModel(Model):
  ...
  def create_feed_dict(self, input_batch, label_batch):
    ...
    if label_batch is None:
      ...
      #label_one_hot = np.zeros((self.config.batch_size, self.config.n_classes))
      #label_one_hot[np.arange(self.config.batch_size), label_batch] = 1
  ...
  def add_model(self, input_data):
    ...
    """
    ### YOUR CODE HERE
    with tf.variable_scope("softmax", initializer=tf.constant_initializer(0.0)):
      weights = tf.get_variable("weights", [self.config.n_features, self.config.n_classes])
      biases = tf.get_variable("biases", [self.config.batch_size, self.config.n_classes])
